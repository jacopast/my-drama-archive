...
def analyze_content(title, combined_comment):
    ...
    # [수정] 따옴표 오류 방지를 위해 한 줄씩 작성
    prompt = f"작품명: '{title}'\n"
    prompt += f"누적 코멘트: '{combined_comment}'\n"
    prompt += "위 정보를 바탕으로 JSON 형식으로만 답해줘.\n"
    prompt += "1. platform: Netflix, Disney+, Prime Video, Apple TV+, Watcha, TVING, Wavve, Cinema 중 1개.\n"
    prompt += "2. rating: 전체 뉘앙스 분석하여 1.0~5.0 사이 점수 (0.5 단위).\n"
    prompt += "3. release_date: 최초 공개일 (YYYY-MM-DD).\n"
    prompt += "4. running_time: 총 러닝타임(분 단위). 숫자만.\n"
    prompt += "5. cast_crew: 주요 감독 1명과 주연 배우 2~3명을 콤마(,)로 구분해 한국어로. (예: 봉준호(연출), 송강호)\n"
    prompt += "JSON 예시:\n"
    prompt += '{"platform": "Netflix", "rating": 4.5, "release_date": "2025-01-01", "running_time": 130, "cast_crew": "이응복(연출), 김고은"}'
